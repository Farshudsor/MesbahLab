...
## Case2 Function
##################
def ce_mpc(F_ode,n_pred,n_ctrl,n_st,n_par_st,n_par_ip,n_ip,uk_lb,uk_ub,xk_lb,xk_ub,xk,theta_par,uk,Tsamp,xkh0,xk_sp,uk_sp,Wx,Wu,uk_opt):
    ...
    for i in range(n_pred):
        ...
        xk_end = x_end['xf']
        Jce = Jce +  mtimes((Uk-uk_sp).T,mtimes(Wu,(Uk-uk_sp))) + mtimes((Xk-xk_sp).T,mtimes(Wx,(Xk-xk_sp)))
...
## Case3 Function
##################
def gencost(n_st,n_ip,n_op,n_par,n_pred,Sigmak,mdl_ode,fy,xk,uk,thetah,Tsamp,xk_sp,uk_sp,Wx,Wu,Q,Qz,R,uk_ce,xkh0,theta_par,xk_ce):
    ...
    de1 = (uk[0]/10/V)*(Cain-xk[0]) - theta_par[0]*exp(-EaR/(xk[1]))*xk[0]
    de2 = (uk[0]/10/V)*(Tin-xk[1]) - theta_par[1]/(rho*cp)*theta_par[0]*exp(-EaR/(xk[1]))*xk[0] + UA/(rho*cp*V)*(uk[1] - xk[1] )
...
#    Cz = jacobian(fy,vertcat(zk))
    ...
    for i in reversed(range(n_pred)):
        if i==(n_pred-1):
            ...
            #Wgi = horzcat(vertcat(Wx,Ktx),vertcat(Ktx.T,Ktt))
