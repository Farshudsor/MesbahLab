...
def robust_dual(n_st,n_ip,n_op,n_par,n_pred,Sigmak,mdl_ode,fy,xk,uk,thetah,Tsamp,Q,Qz,R,uk_ce,xkh0,theta_par,xk_ce,xk_lb,xk_ub):
    ...
    for j in range(n_pred):
        if j==0:
            ...
            Ru = SX.zeros(n_st,n_st)
            Rl = SX.zeros(n_st,n_st)
            for i in range(n_st):
                 Ru[i,i] = (xk[i]>xk_ub[i])
                 Rl[i,i] =  (xk[i]<xk_lb[i])
            ...
            viol_dev_u = (xk-xk_ub)
            viol_dev_l = (xk_lb-xk)
            violation_cost = viol_pen*( mtimes(viol_dev_u.T,mtimes(Ru,viol_dev_u)) + mtimes(viol_dev_l.T,mtimes(Rl,viol_dev_l)))
