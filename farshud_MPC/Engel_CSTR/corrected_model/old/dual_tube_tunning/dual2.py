...
## Case2 Function
##################
def ce_mpc(F_ode,n_pred,n_ctrl,n_st,n_par,n_ip,uk_lb,uk_ub,xk_lb,xk_ub,xk,theta_par,uk,Tsamp,xkh0,uk_opt,xk_ref,uk_ref,k):
...
## Case3 Function
##################
def gencost(n_st,n_ip,n_op,n_par,n_pred,Sigmak,mdl_ode,fy,xk,uk,thetah,Tsamp,Q,Qz,R,uk_ce,xkh0,theta_par,xk_ce,xk_ref,uk_ref,k10):
    ...
    for i in reversed(range(n_pred)):
        if i==(n_pred-1):
            ...
            #Wx[0,0] = -1
            #Wx[3,3] = -1
            Wx = np.diag([-1,0,0,-1,-1,0,0])

            #import pdb; pdb.set_trace()
            #xk_sp = np.array( [1,0,0,1,1,0,0])
            xk_sp = xk_ref[k10+i,:]
