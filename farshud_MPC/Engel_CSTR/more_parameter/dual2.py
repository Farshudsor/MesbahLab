...
def gencost(n_st,n_ip,n_op,n_par,n_pred,Sigmak,mdl_ode,fy,xk,uk,thetah,Tsamp,Q,Qz,R,uk_ce,xkh0,theta_par,xk_ce):
    ...
    de5 = (uk[0]/xk[0])*(Tin - xk[4]) - (theta_par[3]*((2*xk[0]/r)-pi*r**2)*(xk[4]-xk[5]))/(rho*theta_par[2]*xk[0]) - (theta_par[0]*xk[1]*xk[2]*theta_par[1])/(rho*theta_par[2])
    de6 = (Vjin/Vj) * (xk[6]-xk[5]) + (theta_par[3]*((2*xk[0]/r)-pi*r**2)*(xk[4]-xk[5]))/(rho*theta_par[2]*Vj)
    de7 = (1/theta_par[4])* (uk[1]-xk[6])
    ...
    for i in reversed(range(n_pred)):
        if i==(n_pred-1):
            ...
            Wx = cost_h(res_xkh_nom[i,:],uk_pred_nom[i,:]-uk_pred_nom[i-1,:])*0
            ...
            #(3.10)- Adaptive
            #import pdb; pdb.set_trace()
            #H0  = mtimes(res_xkh_nom[i,:],mtimes(Wx,res_xkh_nom[i,:].T))**.5 + mtimes( uk_pred_nom[i,:],mtimes(Wu,uk_pred_nom[i,:].T)) + mtimes(p0.T,res_xkh_nom[i,:].T)
            #H0u = 2*mtimes(Wu,uk_pred_nom[i,:].T)
            #H0x = mtimes(res_xkh_nom[i,:],mtimes(Wx,res_xkh_nom[i,:].T))**(-.5)
            #H0uu=2*Wu
            #H0ux=0
            #H0xx= -.5*mtimes(res_xkh_nom[i,:],mtimes(Wx,res_xkh_nom[i,:].T))**(-1.5)
            ...
            import pdb; pdb.set_trace()
            Ktt = mtimes(mtimes(At.T,K0[(i+1)*n_st:(i+2)*n_st,:]),At) +mtimes(D.T,mtimes(Ktx,At)) + mtimes(mtimes(At.T,Ktx.T),D) + mtimes(mtimes(D.T,Ktt),D) - mtimes(mtimes(mu_0,Ktth).T,Ktth) #3.16
    ...
    for j in range(n_pred):
        if j==0:
            ...
            #T_cost = 98.4-485*T-2.2*T**2 +.302*T**3 -.0165*T**4 +.857*T**5-1.37E-3*T**6 +1.25E-6*T**7 -5E-10*T**8
...
def tube_mpc(F_ode,n_pred,n_ctrl,n_st,n_par,n_ip,uk_lb,uk_ub,xk_lb,xk_ub,xk,theta_par,uk,Tsamp,xkh0,uk_opt,xk_ref,uk_ref,k):
    ...
    for i in range(n_pred):
        ...
        x_end = F_ode(x0=Xk, p=vertcat(theta_par,Uk))
        xk_end = x_end['xf']
        Jce = Jce + mtimes(mtimes((Xk- xk_ref[k+i,:]).T,Qx),(Xk- xk_ref[k+i,:])) + .1*(Uk[0]-uk_ref[k+i,0])**2 + 10**(-6)*(Uk[1]-uk_ref[k+i,1])**2
...
def tube_dual(n_st,n_ip,n_op,n_par,n_pred,Sigmak,mdl_ode,fy,xk,uk,thetah,Tsamp,Q,Qz,R,uk_ce,xkh0,theta_par,xk_ce,xk_ref,uk_ref,k10):
    ...
    for i in reversed(range(n_pred)):
        if i==(n_pred-1):
            ...
            #p0 = mtimes(Ah.T,mtimes((SX.eye(n_st)- mtimes(mu_0,mtimes( K0[(i+1)*n_st:(i+2)*n_st,:], mtimes(Bh,Bh.T)))),p0)) - mtimes(Wx,xk_sp)
            #(3.11) - adaptive
            p01 = H0x - (mtimes(Bh.T,(mtimes(K0[(i+1)*n_st:(i+2)*n_st,:],Ah) + H0ux))).T
            p02 = mtimes(inv(H0uu + (mtimes(Bh.T,(mtimes(K0[(i+1)*n_st:(i+2)*n_st,:],Bh))))) ,H0u)
            p0 = mtimes(p01,p02);
