...
###
###    Reduce number of parameter estimates to 2, using t1 and t5 (index 1 & 4)
###
...
# Run Case 1, 2, and 3;         Case 1 = optimal(all parameters known); Case 2 = CE control; Case 3 = Dual Control
runC1 = True
runC2 = True
runC3 = True

#Define the controller and simulation ranges
run_time = 20
Tsamp = 1
n_pred = 20
n_ctrl = 20
...
n_st = 3 #states
n_ip = 1 #control inputs
n_op = 3 #observed states
n_par_st = 1 # theta 1
n_par_ip = 1 #theta 5
...
if runC3:
    ...
    #theta_h = [1, -0.6, 0.3, 0.1, 0.7, 1.5]   # initial guess
    #Sigma_tt = np.diag([0.1, 0.1, 0.01, 0.01, 0.01, 0.1])
    theta_h = [1, 0.7]   # initial guess
    Sigma_tt = np.diag([0.1, 0.01])
    Q = np.diag([1,1,1])
    Qz = np.diag([1, 1, 1, 0.1, 0.01 ])
    R = np.diag([1,1,1])
    ...
    de1 = xk3[1] + theta_act[3]*uk3[0]
    de2 = xk3[2] + thetah3[1]*uk3[0]
    de3 = thetah3[0]*xk3[0] + theta_act[1]*xk3[1] + theta_act[2]*xk3[2] +  theta_act[5]*uk3[0]
    dthetah1 = [0]*thetah3[0]
    dthetah5 = [0]*thetah3[1]
    ...
    mdl_ode =vertcat(de1,de2,de3,dthetah1,dthetah5)
    ...
    Sigmak_p = np.diag([1, 1,1, 0.1, 0.01])
    ...
    for k in range(run_time):
        ...
        xce2 = ce_res[1::4]
        xce3 = ce_res[2::4]
        xk_ce_3 = horzcat(xce1,xce2,xce3)
        uk_ce_3 = ce_res[3::4]
        uk_opt = uk_ce_3[0]
        ...
        Jd, qu, op = Dual.gencost(n_st,n_ip,n_op,n_par,n_pred,Sigmak,mdl_ode,fy,xk3,uk3,thetah3,Tsamp,xk_sp,Wx,Wu,Q,Qz,R,uk_ce_3,xkh0,F_ode,theta_par,xk_ce_3)
        ...
        #Simulate the system
        x_end = F_ode(x0=xkp, p=vertcat(theta_act_u,uk_opt))
        xkp = x_end['xf'].full()
        xkp = xkp.T# + wkp[k,:]
        ...
        if k == run_time-1:
            Cost_dual = 0.5*(Cost_dual +  sum((res3_xk[-1,:]-rho_v)**2))
...
if runC1:
    ...
    #Define the system equations with unknown parameters
    de1 = xk[1] + theta_act[3]*uk[0]
    de2 = xk[2] + thetah[1]*uk[0]
    de3 = thetah[0]*xk[0] + theta_act[1]*xk[1] + theta_act[2]*xk[2] +  theta_act[5]*uk[0]
    dthetah1 = [0]*thetah[0]
    dthetah5 = [0]*thetah[1]
    #Define the system equations with known parameters
    e1 = xk[1] + .3*uk[0]
    ...
    ode = {'x':xk, 'p':vertcat(uk), 'ode':c1_mdl}
    F_ode = integrator('F_ode', 'cvodes', ode)
    ...
    for i2 in range(run_time):
        ...
        if i2 < run_time-1:
            uk_opt = u_opt[0]
            #Simulate the system
            x_end = F_ode(x0=xkp, p=vertcat(uk_opt))
            xkp = x_end['xf'].full().flatten()
            xkp = xkp.T #+ wkp[k,:]

            #KF prediction
            Ax = Jx(xkh0,uk_opt)
            x_end = F_ode(x0=xkh0, p=vertcat(uk_opt))
            xkh0 = x_end['xf'].full()
...
if runC2:
    ...
    #Define the system equations with unknown parameters
    de1 = xk2[1] + theta_act[3]*uk2[0]
    de2 = xk2[2] + thetah2[1]*uk2[0]
    de3 = thetah2[0]*xk2[0] + theta_act[1]*xk2[1] + theta_act[2]*xk2[2] +  theta_act[5]*uk2[0]
    dthetah1 = [0]*thetah2[0]
    dthetah5 = [0]*thetah2[1]
    #Define the system equations with known parameters
    e1 = xk2[1] + .3*uk2[0]
    e2 = xk2[2] + .5*uk2[0]
    ...
    for k in range(run_time):
        ...
        if k == run_time - 1:
            Cost_ce = 0.5*(Cost_ce + sum((res2_xk[-1,:]-rho_v)**2))

#import pdb; pdb.set_trace()
#if p_plots:
#    P.PlotC2(res3_uk,res3_xk,run_time,(res3_theta-theta_act_u)**2,'Case 3')
#if p_plots:
#    P.PlotC2(res2_uk,res2_xk,run_time,(res2_theta-theta_act_u)**2,'Case 2')
if p_plots:
    P.PlotC2C3(res3_uk,res3_xk,(res3_theta-theta_act_u)**2,'Case 3', res2_uk,res2_xk,(res2_theta-theta_act_u)**2, 'Case 2', run_time)


print 'Cost_dual = ',Cost_dual
print '            final stage = ', res3_xk[-1,:]
print '             est. error sq = ', (theta_act_u - res3_theta[-1,:])**2
#print '             control variation (u_ce - u_dual) = ', Dual_vs_CE
print 'Cost_ce = ', Cost_ce
print '             est. error sq = ', (theta_act_u - res2_theta[-1,:])**2

print' Relative Error (ce - dual) = ' ,(res2_theta[-1,:] - theta_act_u)**2 - (res3_theta[-1,:] - theta_act_u)**2
