...
end_Disturbance = 85 # k0 has been reduced by 10%
slope_Disturbance = (7.2*(10**10)/10 - (6.48*10**10)/10 ) / (end_Disturbance - Disturbance)

Disturbance2 = 25 # time when Delta H begins to be reduced
end_Disturbance2 = 85 #  Delta H has been reduced by 10%
slope_Disturbance2 = ( -5.*(10**4) - -5.*(10**4) *.9 ) / (end_Disturbance2 - Disturbance2)
...
def Mkplots(time, res1_xk, res1_uk, res1_theta, case, a,b,c,set, T_act = act_theta):
    ...
    if c:
        ...
        plt.plot(time,((res1_theta[:,0]- act_theta[:,0])/act_theta[:,0])*100)
        #plt.legend(['$k_0$ estimate'])
        plt.grid()
        plt.subplot(2, 1, 2)
        #plt.ylim(-5.*(10**4) - 1, -5.*(10**4)+1)
        plt.ylabel('$\Delta H$ %error')
        plt.plot(time,((res1_theta[:,1]-act_theta[:,1])/act_theta[:,1])*100)
...
if runC3:
    ...
    Qz = np.diag([10**-2, 10**-1, 9*10**18,10**10])# 10**-1, 9*10**18, 10**-1])
    R = np.diag([1,1])
...
if runC2:
    ...
    #Parameter statistics (2.3)/(5.3-5.4)
    Q = np.diag([1./.5,1./350*0])
    Qz = np.diag([10**-2, 10**-1, 9*10**18,10**10])# 10**-2, 9*10**18, 10**-1])n
    R = np.diag([1,1])
