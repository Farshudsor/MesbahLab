...
run_time = 40
Disturbance = 10 # time when k0 is reduced by 10%
Tsamp = .1
...
# theta actual for plotting purpouse
theta_act = [7.2*(10**10)/10, -5.*(10**4)] #  [ k0, dH ]
for i in range(run_time):
    if i == Disturbance :
        theta_act[0] = (6.48*10**10)/10
...
def Mkplots(time, res1_xk, res1_uk, res1_theta, case, a,b,c,set, T_act = act_theta):
    ...
    if c:
        ...
        plt.plot(time,((res1_theta[:,0]- act_theta[:,0])/act_theta[:,0])**2)
        plt.legend(['$k_0$ estimate'])
        plt.grid()
        plt.subplot(2, 1, 2)
        #plt.ylim(-5.*(10**4) - 1, -5.*(10**4)+1)
        plt.ylabel('$\Delta H$ %error^2')
        plt.plot(time,((res1_theta[:,1]-act_theta[:,1])/act_theta[:,1])**2 )
...
#    Wu = np.diag([(1./900),(1./300)])*.1
    ...
    uk_opt = np.array([100.,300.])
    xkp = np.array([.50,350])
    theta_par = [theta_act[0],theta_act[1]]
    xkh0 = xkp
...
#print('res2-res1 = ',res2_theta-res3_theta)
#import pdb; pdb.set_trace()
plt.show()
