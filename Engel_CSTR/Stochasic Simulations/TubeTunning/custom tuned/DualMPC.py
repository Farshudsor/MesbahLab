...
#Qz = np.diag([0, 0, 0, 0,  0,  0,  0, (3.0457*10**(-7))*discritize*4E-9, 323.05*.1 ])
...
xk_ub = [.007*alpha_u,  inf, inf, inf, 325*alpha_u, inf, inf]
#xk_lb = [0,      0,  0,   0,   322, 0,   0]
#xk_ub = [.007,  inf, inf, inf, 324, inf, inf]
...
def comp_par_error(time, res2_theta, res3_theta, case2, case3, discritize=discritize):
    time = time*60
    bl = np.ones(run_time)
    theta_act =  [3.0457*10**(-7)*discritize*.85, -323.05*.85] #  [ k0, dH ]
    theta_nom =  [3.0457*10**(-7)*discritize, -323.05] #  [ k0, dH ]
    ...
    g = plt.figure(7)
    plt.clf()
    plt.title('$k_0 \ error^2$')
    plt.ylabel('$k_0 \ error^2$')
    plt.plot(time,(res2_theta[:,0]-bl*theta_act[0])**2,time,(res3_theta[:,0]-bl*theta_act[0])**2)
    plt.xlim(0,1200)
    plt.legend(['CE','Dual'])
    plt.grid()
    ...
    h = plt.figure(8)
    plt.clf()
    plt.title('$\Delta H \ error^2$')
    plt.ylabel('$\Delta H \ error^2$')
    plt.plot(time,(res2_theta[:,1]-bl*theta_act[1])**2,time,(res3_theta[:,1]-bl*theta_act[1])**2)
    plt.xlim(0,1200)
    plt.legend(['CE','Dual'])
    plt.grid()
    ...
    for k in range(run_time):
        ...
        if time_remain < n_pred:
            ...
            if k < run_time-2:
                ...
                res2_uk[k,:] =uk_opt # [uk_opt[0],uk_opt[1]]
                uk_opt[0] = uk_opt[0]*discritize

                res2_xk[k,:] =xkp # [xkp[0,0],xkp[0,1]]
                res2_theta[k,:] =theta_par.T# [theta_par[0],theta_par[1]]
            else:
                res2_xk[k,:] =xkp# [xkp[0,0],xkp[0,1]]
                res2_theta[k,:] =theta_par.T# [theta_par[0],theta_par[1]]
