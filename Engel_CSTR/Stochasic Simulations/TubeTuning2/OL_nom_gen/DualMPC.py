...
if gen_nom:
    ...
    x1 = ce_res[0::n_st+n_ip]
    x2 = ce_res[1::n_st+n_ip]
    x3 = ce_res[2::n_st+n_ip]
    x4 = ce_res[3::n_st+n_ip]
    x5 = ce_res[4::n_st+n_ip]
    x6 = ce_res[5::n_st+n_ip]
    x7 = ce_res[6::n_st+n_ip]
    xk_ref = horzcat(x1,x2,x3,x4,x5,x6,x7)
    u1 = ce_res[7::n_st+n_ip]
    u2 = ce_res[8::n_st+n_ip]
    uk_ref = horzcat(u1,u2)
...
if runC2:
    ...
    for k in range(run_time):
        ...
        Jce, qu_ce, lbq, ubq, g, lbg, ubg, qu_init = Dual.ce_mpc(F_ode,n_pred,n_ctrl,n_st,n_par,n_ip,uk_lb,uk_ub,xk_lb,xk_ub,xk2,theta_par,uk2,Tsamp,xkh0,uk_opt,xk_ref,uk_ref,k)
        qp_mpc = {'x':vertcat(*qu_ce), 'f':Jce, 'g':vertcat(*g)}
        solver_mpc = nlpsol('solver_mpc', MySolver, qp_mpc,{'ipopt':{'max_iter':1000,"check_derivatives_for_naninf":'yes', "print_user_options":'yes' }})
        res_mpc = solver_mpc(x0=qu_init, lbx=lbq, ubx=ubq, lbg=lbg, ubg=ubg)
