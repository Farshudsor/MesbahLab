...
def tube_dual(n_st,n_ip,n_op,n_par,n_pred,Sigmak,mdl_ode,fy,xk,uk,thetah,Tsamp,Q,Qz,R,uk_ce,xkh0,theta_par,xk_ce,xk_ref,uk_ref,k10):
    ...
    #This loop generates the matrices related to Eqn. 12-23 in the write-up
    for i in reversed(range(n_pred)):
        cost = mtimes(mtimes((xk- xk_ref[k10+i,:]).T,Wx_sym),(xk- xk_ref[k10+i,:])) + .1*(uk[0]-uk_ref[k10+i,0])**2 + 10**(-6)*(uk[1]-uk_ref[k10+i,1])**2
        ...
        if i==(n_pred-1):
            ...
            Wxn = np.diag([1,0,0,1,0,0,0])

            #Wxn = np.diag([1,1,1,1,1,1,1])
            ...
            Wx = cost_h(res_xkh_nom[i,:],uk_pred_nom[i,:],Wx)*0.0
            xk_sp = cost_g(res_xkh_nom[i,:],uk_pred_nom[i,:],Wx)
...
def robust_dual(n_st,n_ip,n_op,n_par,n_pred,Sigmak,mdl_ode,fy,xk,uk,slack_u,slack_l,thetah,Tsamp,Q,Qz,R,uk_ce,xkh0,theta_par,xk_ce,xk_lb,xk_ub):
...
#                 Rk[i,i] = (xk[i]>xk_ub[i]) + (xk[i]<xk_lb[i])
            ...
            viol_pen = 0
            dev_u =   (xk_ub + slack_u)-xk >= 0
            dev_l =   xk-(xk_lb - slack_l) >= 0
            Sk = SX([(1./500)*(1./.001**2),0,0,0,(1.0/20**2),0,0])

            #import pdb; pdb.set_trace()
            violation_cost = viol_pen*( dot(Sk,slack_l) + dot(Sk,slack_u) )
