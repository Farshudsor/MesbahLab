...
if runC6:
    ...
    for k in range(run_time):
        ...
        #nlp = {'x':vertcat(*qu), 'f': Jd, 'p': op} #test2
        nlp = {'x':vertcat(*qu), 'f': Jd, 'p': op, 'g':vertcat(dev_u,dev_l)}  #test1
        #import pdb; pdb.set_trace()
        solver = nlpsol('solver', MySolver, nlp, {'ipopt':{'max_iter':1000,"check_derivatives_for_naninf":'yes', "print_user_options":'yes' }})

        u_s_guess = np.append(np.append(uk_opt1,slack_lb), slack_lb)
        u_s_ub = np.append(np.append(uk_ub,slack_ub), slack_ub)
        u_s_lb = np.append(np.append(uk_lb,slack_lb), slack_lb)
        res = solver( x0=u_s_guess, p=xkh0, ubx=u_s_ub, lbx=u_s_lb)
        uk_opt = res['x'].full().flatten()[0:2]
        #if sum(res['x'].full().flatten()[2:]) != 0 :
        #    import pdb; pdb.set_trace()
