...
n_ip =2
n_par=2
row_per = (1+n_st*3 + 1+n_ip*3 + 1+n_par*3)
discritize = 60 *2
...
def Mkplots(n_sim, xk, uk, theta, case2, x_opt, u_opt, t_opt):
    ...
    plt.step(time,bl*bounds[2]*uk_ub[0],'r-',alpha=opacity)
    plt.step(time,bl*bounds[3]*uk_lb[0],'r-',alpha=opacity)
    ...
    plt.step(time,bl*uk_ub[0]*bounds[2],'r-',alpha=opacity)
    plt.step(time,bl*uk_lb[0]*bounds[3],'r-',alpha=opacity)
...
for i in np.arange(0, n_sim):
    ...
    res3_xk[i*n_st,:] = All_data[i*row_per+3][2].replace('[','').replace(']','').replace('\n','').split()
    res_xk[i*n_st+1,:] = All_data[i*row_per+4][2].replace('[','').replace(']','').replace('\n','').split()
    res2_xk[i*n_st+1,:] = All_data[i*row_per+5][2].replace('[','').replace(']','').replace('\n','').split()
    res3_xk[i*n_st+1,:] = All_data[i*row_per+6][2].replace('[','').replace(']','').replace('\n','').split()
    res_xk[i*n_st+2,:] = All_data[i*row_per+7][2].replace('[','').replace(']','').replace('\n','').split()
    res2_xk[i*n_st+2,:] = All_data[i*row_per+8][2].replace('[','').replace(']','').replace('\n','').split()
    res3_xk[i*n_st+2,:] = All_data[i*row_per+9][2].replace('[','').replace(']','').replace('\n','').split()
    res_xk[i*n_st+3,:] = All_data[i*row_per+10][2].replace('[','').replace(']','').replace('\n','').split()
    res2_xk[i*n_st+3,:] = All_data[i*row_per+11][2].replace('[','').replace(']','').replace('\n','').split()
    res3_xk[i*n_st+3,:] = All_data[i*row_per+12][2].replace('[','').replace(']','').replace('\n','').split()
    res_xk[i*n_st+4,:] = All_data[i*row_per+13][2].replace('[','').replace(']','').replace('\n','').split()
    res2_xk[i*n_st+4,:] = All_data[i*row_per+14][2].replace('[','').replace(']','').replace('\n','').split()
    res3_xk[i*n_st+4,:] = All_data[i*row_per+15][2].replace('[','').replace(']','').replace('\n','').split()
    res_xk[i*n_st+5,:] = All_data[i*row_per+16][2].replace('[','').replace(']','').replace('\n','').split()
    res2_xk[i*n_st+5,:] = All_data[i*row_per+17][2].replace('[','').replace(']','').replace('\n','').split()
    res3_xk[i*n_st+5,:] = All_data[i*row_per+18][2].replace('[','').replace(']','').replace('\n','').split()
    res_xk[i*n_st+6,:] = All_data[i*row_per+19][2].replace('[','').replace(']','').replace('\n','').split()
    res2_xk[i*n_st+6,:] = All_data[i*row_per+20][2].replace('[','').replace(']','').replace('\n','').split()
    res3_xk[i*n_st+6,:] = All_data[i*row_per+21][2].replace('[','').replace(']','').replace('\n','').split()

    res_theta[i*n_par,:] = All_data[i*row_per+23][2].replace('[','').replace(']','').replace('\n','').split()
    res2_theta[i*n_par,:] = All_data[i*row_per+24][2].replace('[','').replace(']','').replace('\n','').split()
    res3_theta[i*n_par,:] = All_data[i*row_per+25][2].replace('[','').replace(']','').replace('\n','').split()
    res_theta[i*n_par+1,:] = All_data[i*row_per+26][2].replace('[','').replace(']','').replace('\n','').split()
    res2_theta[i*n_par+1,:] = All_data[i*row_per+27][2].replace('[','').replace(']','').replace('\n','').split()
    res3_theta[i*n_par+1,:] = All_data[i*row_per+28][2].replace('[','').replace(']','').replace('\n','').split()

    res_uk[i*n_par,:] = All_data[i*row_per+30][2].replace('[','').replace(']','').replace('\n','').split()
    res2_uk[i*n_par,:] = All_data[i*row_per+31][2].replace('[','').replace(']','').replace('\n','').split()
    res3_uk[i*n_par,:] = All_data[i*row_per+32][2].replace('[','').replace(']','').replace('\n','').split()
    res_uk[i*n_par+1,:] = All_data[i*row_per+33][2].replace('[','').replace(']','').replace('\n','').split()
    res2_uk[i*n_par+1,:] = All_data[i*row_per+34][2].replace('[','').replace(']','').replace('\n','').split()
    res3_uk[i*n_par+1,:] = All_data[i*row_per+35][2].replace('[','').replace(']','').replace('\n','').split()
