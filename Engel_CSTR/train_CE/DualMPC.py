...
csv_file2 = "./results/CE_training_Input.csv"
csv_fileT = "./results/CE_training_T.csv"
csv_fileV = "./results/CE_training_V.csv"
csv_fileC = "./results/CE_training_C.csv"
csv_file3 = "./results/"+testname+"_time.csv"

#csv_file = "./results/"+testname+"_"+run_name+"_val.csv"
#csv_file2 = "./results/CE_val_Input.csv"
#csv_fileT = "./results/CE_val_T.csv"
#csv_fileV = "./results/CE_val_V.csv"
#csv_fileC = "./results/CE_val_C.csv"
#csv_file3 = "./results/"+testname+"_time_val.csv"

save2csv = True
zz =640 - 480 +6+100
#630 training + 50 zero noise
...
vkp = (np.random.randn(run_time,n_st))*0
vkp = mtimes(vkp,R)
...
rand1 = np.random.randn()*.10+1
rand2 = np.random.randn()*.10+1

theta_nom = np.array([2.5889*10**(-7)*discritize, -274.5925]) #  [ k0, dH ]
theta_act = np.array([2.5889*10**(-7)*discritize*rand1, -274.5925*rand2]) #  [ k0, dH ]
...
res_theta = np.ones((run_time+1,n_par))*theta_act
...
def writeData_2(solve_time, vkp,res_uk,res_xk,res_theta,res2_uk,res2_xk,res2_theta,
                ...
                run_time,csv_file, theta_act=theta_act):
    ...
    with File2:
        ...
        tempx = []
        tempx += [[float(vkp[i,k]) for i in range(run_time)] for k in range(n_st)]
        tempx = vertcat(theta_act,np.asarray(tempx).reshape(70,1))
        tempx = [float(tempx[i]) for i in range(72)]

        writer.writerow(tempx[:])#, res2_xk[:,0], [res2_xk[:,0].T*res2_xk[:,3]]])

    FileT = open(csv_fileT, 'a')
    with FileT:
        writer = csv.writer(FileT)
        writer.writerow([res2_xk[i,4] for i in range(run_time)])#, res2_xk[:,0], [res2_xk[:,0].T*res2_xk[:,3]]])
    FileV = open(csv_fileV, 'a')
    with FileV:
        writer = csv.writer(FileV)
        writer.writerow([res2_xk[i,0] for i in range(run_time)])#, res2_xk[:,0], [res2_xk[:,0].T*res2_xk[:,3]]])
    FileC = open(csv_fileC, 'a')
    with FileC:
        writer = csv.writer(FileC)
        writer.writerow([res2_xk[i,0]*res2_xk[i,3] for i in range(run_time)])#, res2_xk[:,0], [res2_xk[:,0].T*res2_xk[:,3]]])
