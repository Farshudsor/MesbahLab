...
n_sim = 125      #verify n_sim from DualMPC.py
run_time=10
n_st=7
n_ip =2
n_par=2
...
res_theta = np.zeros((n_sim*n_par,run_time) )
...
def Mkplots(n_sim, xk, uk, theta, case2, x_opt, u_opt, t_opt):
    ...
    for i in np.arange(0, n_sim):
        ...
        plt.plot(time[0:-1],(res1_theta[0,0:-1]-opt_t[0,:])**2,'b-',alpha=opacity)
        #plt.plot(time,opt_t[0,:],'p:')
        #plt.plot(time,res_theta[:,0],'g--')
    ...
    for i in np.arange(0, n_sim):
        ...
        res1_theta = theta[i*n_par:(i+1)*n_par,:]
        opt_t = t_opt[i*n_par:(i+1)*n_par,:]
        plt.plot(time[0:-1],(res1_theta[1,0:-1]-opt_t[1,:])**2,'b-',alpha=opacity)
        #plt.plot(time,opt_t[1,:],'p:')
        #plt.plot(time,res_theta[:,1],'g--')
...
def comp_par_error(n_sim, res2_theta, res3_theta):
    ...
    bl = np.ones(run_time)
    theta_act =  [3.0457*10**(-7)*discritize*.85, -323.05*.85] #  [ k0, dH ]
    theta_nom =  [3.0457*10**(-7)*discritize*.85, -323.05*.85] #  [ k0, dH ]
...
def constraint_violations(n_sim, xk, uk, theta, case2):
    uk_lb = [0,                     280]
    ...
    Violation_count += sum(sum(xk[:,:][0::n_st]>.007))
    Violation_count += sum(sum(xk[:,:][4::n_st]>325))
    Violation_count += sum(sum(xk[:,:][4::n_st]<321))

#    import pdb; pdb.set_trace()

    avg = sum(xk[:,:][0::n_st]*xk[:,:][3::n_st])[10]/n_sim
    std = (sum((xk[:,:][0::n_st]*xk[:,:][3::n_st] - (sum(xk[:,:][0::n_st]*xk[:,:][3::n_st])[10]/n_sim))**2)[10])**.5


    print (case2+" has "+str(np.round(avg,4))+" avg +/- "+str(np.round(std,4))+" SD moles of C on avg"+" and "+ str(Violation_count)+ " instances of state constraint violation" )
...
constraint_violations(n_sim, res2_xk, res2_uk, res2_theta, case2)
constraint_violations(n_sim, res3_xk, res3_uk, res3_theta, case3)
constraint_violations(n_sim, res4_xk, res4_uk, res4_theta, case4)
constraint_violations(n_sim, res5_xk, res5_uk, res5_theta, case5)

Mkplots(n_sim, res2_xk, res2_uk, res2_theta, case2, res_xk*0, res_uk*0,res_theta)
Mkplots(n_sim, res3_xk, res3_uk, res3_theta, case3, res_xk*0, res_uk*0,res_theta)
