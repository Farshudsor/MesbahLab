...
theta_nom = np.array([3.0457*10**(-7)*discritize, -323.05, 4.2*1.2, .14844*discritize*1.2 , 900/discritize*1.2]) #  [ k0, dH ]
theta_act = np.array([3.0457*10**(-7)*discritize*.85, -323.05*.85, 4.2, .14844*discritize, 900/discritize]) #  [ k0, dH ]

#Parameter statistics (2.3)/(5.3-5.4)
Q = np.diag([0, 0, 0, 0,  0,  0,  0])
Qz = np.diag([0, 0, 0, 0,  0,  0,  0, ((3.0457*10**(-7))*discritize*.2)**2, (323.05*.2)**2, (4.2*.2)**2, (.14844*discritize*.2)**2, (900/discritize*.2)**2])
...
def writeData_2(res_uk,res_xk,res_theta,res2_uk,res2_xk,res2_theta,res3_uk,res3_xk,res3_theta,res4_uk,res4_xk,res4_theta,res5_uk,res5_xk,res5_theta,run_time,csv_file):
...
def writeData_1(res2_uk,res2_xk,res2_theta,res3_uk,res3_xk,res3_theta,run_time,csv_file):
    File = open(csv_file, 'a')
    with File:
        writer = csv.writer(File)
        writer.writerow(['   States   '])
        ...
        writer.writerow([ 'CE', 'Delta H', res2_theta[:,1]])
        writer.writerow([ 'Dual', 'Delta H', res3_theta[:,1]])
        writer.writerow([ 'CE', 'cp', res2_theta[:,2]])
        writer.writerow([ 'Dual', 'cp', res3_theta[:,2]])
        writer.writerow([ 'CE', 'U', res2_theta[:,3]])
        writer.writerow([ 'Dual', 'U', res3_theta[:,3]])
        writer.writerow([ 'CE', 'tau_c', res2_theta[:,4]])
        writer.writerow([ 'Dual', 'tau_c', res3_theta[:,4]])
...
if runC2:
    ...
    de5 = (uk2[0]/xk2[0])*(Tin - xk2[4]) - (thetah2[3]*((2*xk2[0]/r)-pi*r**2)*(xk2[4]-xk2[5]))/(rho*thetah2[2]*xk2[0]) - (thetah2[0]*xk2[1]*xk2[2]*thetah2[1])/(rho*thetah2[2])
    de6 = (Vjin/Vj) * (xk2[6]-xk2[5]) + (thetah2[3]*((2*xk2[0]/r)-pi*r**2)*(xk2[4]-xk2[5]))/(rho*thetah2[2]*Vj)
    de7 = (1/thetah2[4])* (uk2[1]-xk2[6])
    ...
    mdl_ode =vertcat(de1,de2,de3,de4,de5,de6,de7,dthetah1,dthetah2,dthetah3,dthetah4,dthetah5)
    mdl_ode2 =vertcat(de1+wk2[0],de2+wk2[1],de3+wk2[2],de4+wk2[3],de5+wk2[4],de6+wk2[5],de7+wk2[6],dthetah1,dthetah2,dthetah3,dthetah4,dthetah5)
...
if runC3:
    ...
    de5 = (uk3[0]/xk3[0])*(Tin - xk3[4]) - (thetah3[3]*((2*xk3[0]/r)-pi*r**2)*(xk3[4]-xk3[5]))/(rho*thetah3[2]*xk3[0]) - (thetah3[0]*xk3[1]*xk3[2]*thetah3[1])/(rho*thetah3[2])
    de6 = (Vjin/Vj) * (xk3[6]-xk3[5]) + (thetah3[3]*((2*xk3[0]/r)-pi*r**2)*(xk3[4]-xk3[5]))/(rho*thetah3[2]*Vj)
    de7 = (1/thetah3[4])* (uk3[1]-xk3[6])
    ...
    sys_ode = vertcat(de1,de2,de3,de4,de5,de6,de7)
    mdl_ode =vertcat(de1,de2,de3,de4,de5,de6,de7,dthetah1,dthetah2,dthetah3,dthetah4,dthetah5)
    mdl_ode2 =vertcat(de1+wk3[0],de2+wk3[1],de3+wk3[2],de4+wk3[3],de5+wk3[4],de6+wk3[5],de7+wk3[6],dthetah1,dthetah2,dthetah3,dthetah4,dthetah5)
...
if runC2_tube:
    ...
    Ju4 = Function('Ju4',[xk4,uk4,thetah4],[jacobian((vertcat(xk4,thetah4)+mdl_ode),uk2)])
...
if save2csv:
    #writeData_2(res_uk,res_xk,res_theta,res2_uk,res2_xk,res2_theta,res3_uk,res3_xk,res3_theta,res4_uk,res4_xk,res4_theta,res5_uk,res5_xk,res5_theta,run_time,csv_file)
    writeData_1(res2_uk,res2_xk,res2_theta,res3_uk,res3_xk,res3_theta,run_time,csv_file)
