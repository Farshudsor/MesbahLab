...
if gen_nom:
    for j7 in range(sweep_vals):
        for j8 in range(sweep_vals):
            ...
            theta_act = [theta_nom[0]*sweep_vals[j7], theta_nom[1]*sweep_vals[j8]]
            ...
            for k in range(run_time):
                ...
                #Simulate the system
                x_end = F_ode(x0=xkp, p=vertcat(theta_nom,uk_opt))
                xkp = x_end['xf'].full()
                xkp = xkp.T #+ wkp[k,:]
            ...
            with File:
                ...
                writer.writerow(['   k0  ', '  Delta H   ', 'Nominal Trajectory' ])
                writer.writerow([ theta_act[0], theta_act[1] ,res_xk[:,0]])
