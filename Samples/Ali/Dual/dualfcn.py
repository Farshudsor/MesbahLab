...
def gencost(n_st,n_ip,n_op,n_par,n_pred,Sigmak,mdl_ode,fy,xk,uk,thetah,Tsamp,xk_sp,Wx,Wu,Q,Qz,R,uk_ce):
    ...
    for i in reversed(range(n_pred)):
        if i==(n_pred-1):
            ...
            # p0x = mtimes(Wx,(res_xkh_nom[i,:].T - xk_sp))
        else:
            ...
            #for b1 in range(n_st):
             #   e_base_sum = e_base_sum + mtimes(mtimes(e_base[:,b1].reshape(1,4),p0x),Bh[b1,:])
    ...
    for j in range(n_pred):
        if j==0:
            ...
            for b1 in range(n_st):
                ...
                e_base_vec = e_base_vec + mtimes(e_base[:,b1],trace((mtimes(Hx,Sigmak[0:n_st,0:n_st]))))

            x_end = Fx(res_xkh_nom[j,:].T,uk,thetah) #mtimes(Ah,res_xkh_nom[j,:].T) + mtimes(Bh,uk)
            x_end = x_end[0:n_st] + 0.5*e_base_vec
            xk = x_end[0:n_st]
...
def ce_nmpc(mdl_ode,n_pred,n_ctrl,n_st,n_par,n_ip,uk_lb,uk_ub,xk_lb,xk_ub,xk,thetah,uk,Tsamp,zkh0,xk_sp,Wx):
    Jce = 0.0
    zk = vertcat(xk,thetah)
    ...
    for i in range(n_pred):
        ...
        qu_ce += [vertcat(xk,thetah)]
        lbq += [vertcat(xk_lb,-inf*NP.ones((n_par,1)))]
        ubq += [vertcat(xk_ub,inf*NP.ones((n_par,1)))]
        qu_init += zkh0.full().tolist()
